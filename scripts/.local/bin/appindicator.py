...
def get_weather_data():
    ...
    try:
        weather = requests.get("https://wttr.in/Krasnodar?format=j1", timeout=20).json()
    except Exception:
...
def change_wallpaper(widget, next=None):
    ...
    if len(walls) <= CURR_WALL:
        CURR_WALL = 0
    subprocess.call([WALLS_BINARY, WALLS_BINARY_ARG, str(walls[CURR_WALL])])
